...
We evaluate $R_0$ at specific parameter values by substituting into the
symbolic expression:

``` julia
R0_val = Symbolics.value(
    Symbolics.substitute(R0_sir, Dict(β => 0.6, γ => 0.1, κ => 5.0))
)
println("R₀ = ", R0_val)
println("Expected: βκ/(β+γ) = ", 0.6 * 5.0 / (0.6 + 0.1))
```

    R₀ = 4.285714285714286
    Expected: βκ/(β+γ) = 4.285714285714286

With $\beta = 0.6$, $\gamma = 0.1$, and $\kappa = 5$, we get
$R_0 = 3/0.7 \approx 4.29$, confirming the analytic formula.
...
R0_seir_val = Symbolics.value(
    Symbolics.substitute(R0_seir, Dict(β => 0.6, γ => 0.1, σ => 0.2, κ => 5.0))
)
...
```
    ...
    SEIR R₀ = 4.285714285714286
    SIR  R₀ = 4.285714285714286
    Equal? true
...
R0_vs_κ = [
    Symbolics.value(Symbolics.substitute(R0_sir, Dict(β => 0.6, γ => 0.1, κ => κ_v)))
    for κ_v in κ_range
]
...
Figure 1: R₀ as a function of mean network degree κ, for fixed β = 0.6
and γ = 0.1. The dashed line marks the epidemic threshold R₀ = 1.

</div>

For a Poisson network with $R_0 = \beta\kappa/(\beta + \gamma)$, the
epidemic threshold $R_0 = 1$ occurs at
$\kappa^* = (\beta + \gamma)/\beta$. With $\beta = 0.6$ and
$\gamma = 0.1$, this gives $\kappa^* = 0.7/0.6 \approx 1.17$.
...
R0_vs_β = [
    Symbolics.value(Symbolics.substitute(R0_sir, Dict(β => β_v, γ => 0.1, κ => 5.0)))
    for β_v in β_range
]
...
Figure 2: R₀ as a function of transmission rate β, for fixed κ = 5 and γ
= 0.1.
