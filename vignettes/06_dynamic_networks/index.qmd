...
The static case uses a standard `gillespie_ribbon`; the dynamic case
uses `TimeVaryingNetwork` from `NetworkOutbreaks.jl` where each edge
independently toggles between active and dormant: active edges break
at rate $\eta_2$ and dormant edges reform at rate $\eta_1$.
...
params_no = Dict(:β => β_val, :γ => γ_val)

# --- Static baseline ---
t_st, μ_st, σ_st = gillespie_ribbon(prog_no, params_no,
...
# --- Dynamic: pre-generate edge break/reform schedule ---
...
function dynamic_poisson_builder(N, κ, η₁, η₂, tmax; rng_seed = 20240601)
    return function (rng)
        ...
        updates = NamedTuple{(:t, :src, :dst, :action),
                              Tuple{Float64, Int, Int, Symbol}}[]
        # For each initial edge, schedule break/reform toggling
        for e in edges(g)
            s, d = src(e), dst(e)
            t_now = 0.0
            active = true
...
# Run SSA ensemble on dynamic networks
no_model = OutbreakModel(
    [:S, :I, :R], [false, true, false],
    [OutbreakTransition(:S, :I, β_val, :infection; via = [:I]),
     OutbreakTransition(:I, :R, γ_val, :spontaneous)];
    name = :SIR_dynamic)

n_dyn_runs = 5
nsims_dyn = 40
tgrid_dyn = collect(0.0:0.5:40.0)
I_dyn_runs = Matrix{Float64}(undef, n_dyn_runs * nsims_dyn, length(tgrid_dyn))
row = 1
rng_outer = StableRNG(20240601)
for gi in 1:n_dyn_runs
    net = dyn_builder(rng_outer)
    spec = OutbreakSpec(model = no_model, network = net,
                        initial = SeedFraction(:I => seed_fraction),
                        tspan = (0.0, 40.0))
    ens = simulate_ensemble(spec; nsims = nsims_dyn,
                            seed = 20240601 + 1000 * gi, parallel = false)
    for traj in ens.trajectories
        for (j, t) in enumerate(tgrid_dyn)
            st = state_at(traj, t)
            I_dyn_runs[row, j] = Float64(st[no_model.index_of[:I]]) / N_v
        end
        row += 1
    end
end
...
#| fig-cap: "EBCM (lines) vs SSA ribbons (mean ± 1σ). Left: static network (Poisson κ=5, R₀=2). Right: dynamic network with edge turnover (η₁=0.05, η₂=0.03). Edge dynamics reduce the epidemic peak, and the dynamic EBCM tracks the dynamic SSA."
...
plot!(p1, t_st, μ_st[:I], ribbon = σ_st[:I],
...
plot!(p2, tgrid_dyn, μ_dyn, ribbon = σ_dyn,
...
captures the reduced epidemic driven by edge turnover: breaking contacts
slows transmission, and the SSA on `TimeVaryingNetwork` confirms this
effect (right).
