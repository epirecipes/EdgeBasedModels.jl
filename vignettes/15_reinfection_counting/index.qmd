...
We build an SIS model on a Poisson configuration-model network with mean
degree $\kappa = 5$, transmission rate $\beta = 0.4$, recovery rate
$\gamma = 1.0$, and $L = 3$ reinfection strata.
...
ic     = default_initial_conditions(system; ε = 0.02)
prob   = ODEProblem(system.system, ic, (0.0, 30.0))
...
```{julia}
N = 1000
g = random_regular_graph(N, 5; rng = StableRNG(42))
...
$t = 30$ and saturate counts above $L$:
...
groupedbar([hist_frac pred],
    ...
    title = "Reinfection histogram at t = 30",
    bar_position = :dodge,
