...
```{julia}
ic_multi = default_initial_conditions(model_multi; seed_fraction = 0.01)
...
We compare each ODE prediction (SIR, SEIR, SIS) against Gillespie SSA on a
Poisson network with the same mean degree $\kappa = 5$ via `NetworkOutbreaks.jl`.
...
```{julia}
#| label: fig-validation-sir-seir
#| fig-cap: "Gillespie SSA mean ± 1σ ribbons (matched line colors) versus EBCM I/(E+I) curve for SIR and SEIR."
...
```{julia}
#| label: fig-validation-sis
#| fig-cap: "SIS on Poisson κ=5: EBCM endemic equilibrium versus SSA mean ± 1σ (red ribbon)."
...
The deterministic EBCM curves track the SSA ensemble means well across SIR,
SEIR, and SIS, confirming that the configuration-model closure recovers the
correct large-network limit.
