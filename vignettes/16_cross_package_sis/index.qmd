...
```{julia}
# Benchmark parameters — identical to NBM vignette 13
N        = 500
k        = 3
β_val    = 0.6
γ_val    = 0.4
...
```{julia}
sys_motif = NBM.motif_based_sis(β = β_val, γ = γ_val, k = k, m = 3,
                                tspan  = (0.0, tmax),
                                N      = Float64(N), ε = ε_val,
                                n_p3   = Float64(n_p3_count),
                                n_c3   = Float64(n_triangles))
sol_motif = NBM.solve_motif(sys_motif; saveat = save_dt)
iI_motif  = sys_motif.index[(:singleton, [:I])]
I_motif   = [u[iI_motif] / N for u in sol_motif.u]
@printf("Motif m=3 endpoint I/N = %.5f\n", I_motif[end])
```
...
rows = [
    ...
    ("Motif m=3",                   I_motif[end], I_motif[end] - ref),
