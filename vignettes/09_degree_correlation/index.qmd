...
mean_k2 = sum((k - 1)^2 * probs[k] for k in eachindex(probs))
var_k = mean_k2 - mean_k^2
excess_moment = (mean_k2 - mean_k) / mean_k   # ⟨k²−k⟩/⟨k⟩
println("Mean degree: ", mean_k)
println("Variance:    ", var_k)
println("Excess degree moment: ", excess_moment)
```
...
How does assortativity affect $R_0$? We sweep $r$ from 0 to 1, anchoring at
$R_0 = 2$ for neutral mixing ($r = 0$): transmissibility
$T = 2\langle k\rangle / (\langle k^2\rangle - \langle k\rangle)$,
$\gamma = 0.25$, $\beta = T\gamma/(1-T)$.

```{julia}
γ = 0.25
T = 2.0 / excess_moment                  # R₀ = T·excess_moment = 2 at neutral
β = T * γ / (1 - T)
r_vals = 0.0:0.02:1.0
...
We verify the prediction that **assortative rewiring reduces $R_0$** (and
hence the epidemic final size) by direct stochastic simulation. We build
configuration-model graphs with the bimodal degree distribution
($p_3 = p_7 = 0.5$, $N = 2000$) and then apply Xulvi-Brunet–Sokolov
rewiring with assortativity bias $p_{\text{ass}} \in \{0, 0.25, 0.5, 0.75, 1.0\}$,
preserving the exact degree sequence while increasing the empirical
assortativity coefficient. For each $p_{\text{ass}}$ we run a `DirectSSA`
ensemble with $\beta$ and $\gamma = 0.25$ anchored at $R_0 = 2$ for neutral
mixing and report the empirical final size.

```{julia}
include("../_validation.jl")

γ_v = 0.25
T_v = T                                    # from the R₀=2 anchor above
β_v = T_v * γ_v / (1 - T_v)
...
```{julia}
#| label: fig-assort-validation
#| fig-cap: "Top: analytic R₀ from `correlated_R0` for the bimodal distribution, anchored at R₀=2 for neutral mixing. Bottom: empirical final size from DirectSSA on Xulvi-Brunet–Sokolov rewired bimodal configuration graphs (N=2000, 36 trajectories per p_ass, ribbon = 95% empirical interval). As assortativity increases, the epidemic becomes confined to the high-degree (k=7) class, reducing the population-level final size even as the per-class spectral radius rises."
...
next-generation matrix changes, and the SSA final size responds
correspondingly — confirming that `correlated_R0` operationally
controls the epidemic size on real degree-correlated networks.
