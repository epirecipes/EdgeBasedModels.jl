...
#| fig-cap: "Erlang distributions with the same mean (1/γ = 4) but increasing shape parameter n. As n increases, the distribution concentrates around the mean."
...
ic_exp = default_initial_conditions(model_exp; seed_fraction = 0.01)
prob_exp = ODEProblem(
    model_exp.system,
...
```{julia}
# Solve Erlang SIR
ic_erlang = default_initial_conditions(model_erlang; seed_fraction = 0.01)
...
for n in [1, 2, 5, 10, 20]
    ...
    ic_n = default_initial_conditions(model_n; seed_fraction = 0.01)
    prob_n = ODEProblem(
        model_n.system,
...
for cv in [1.0, 0.5, 0.3, 0.2]
    ...
    ic_cv = default_initial_conditions(model_cv; seed_fraction = 0.01)
    prob_cv = ODEProblem(
        model_cv.system,
...
ic_seir_exp = default_initial_conditions(model_seir_exp; seed_fraction = 0.01)
...
```{julia}
# Solve Erlang SEIR
ic_seir_erl = default_initial_conditions(model_seir_erlang; seed_fraction = 0.01)
...
networks (`NetworkOutbreaks.jl`). Erlang dynamics involve sub-stages that
require a custom multi-stage model in NO; here we focus on the exponential
case, which is the canonical reference.
...
```{julia}
#| label: fig-stages-validation
#| fig-cap: "Exponential EBCM (red line) vs Gillespie SSA mean ± 1σ (red ribbon)."
